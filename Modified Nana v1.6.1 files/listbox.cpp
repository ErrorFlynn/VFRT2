...
namespace nana
{
		...
		namespace listbox
		{
			...
			void es_lister::scroll_into_view(const index_pair& abs_pos, view_action vw_act)
			{
				...
				if (view_action::bottom_view == vw_act)
				{
					...
					if (off >= screen_px)
						origin.y = static_cast<int>(off - screen_px/2);
				}
				else
				{
					auto const content_px = ess_->content_view->content_size().height;
					if (content_px - off >= screen_px/2)
						origin.y = static_cast<int>(off);
					else if (content_px >= screen_px)
						origin.y = static_cast<int>(content_px - screen_px/2);
				}
	...
	//class listbox
		...
		std::pair<listbox::size_type, bool> listbox::sort_col() const
		{
			return{_m_ess().lister.sort_attrs().column, _m_ess().lister.sort_attrs().reverse};
		}
