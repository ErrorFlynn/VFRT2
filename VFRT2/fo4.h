...
wstring GetFO4Path(HWND hwnd = nullptr)
{
	...
	if(steampath.size())
	{
		...
		wstring fo4path{steampath + L"steamapps\\common\\Fallout 4"};
		if(FileExist(fo4path)) return fo4path;
		else
		{
			fo4path = steampath + L"steamapps\\common\\Fallout4";
			if(FileExist(fo4path)) return fo4path;
		}
		...
		for(const auto &folder : folders)
		{
			...
			if(FileExist(fo4path)) return fo4path;
			fo4path = wfolder + L"steamapps\\common\\Fallout4";
			if(FileExist(fo4path)) return fo4path;
		}
	}
	...
	if(fo4path.size() && FileExist(fo4path)) return fo4path;
	...
	MessageBoxA(hwnd, "Failed to retrieve the Fallout 4 path from registry. Please locate the game folder manually. "
